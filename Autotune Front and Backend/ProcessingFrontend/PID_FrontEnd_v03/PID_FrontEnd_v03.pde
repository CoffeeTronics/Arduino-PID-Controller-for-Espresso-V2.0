 ...
 * February 2013
 * Edited by Konstantine @Palanski and Ross Satchell to incorporate easier tuning for espresso machines.
 * Edits and additions marked with //EDIT
 ********************************************************/
...
float InScaleMin = 150;    // set the Y-Axis Min  //EDIT Set your window MIN TEMP here
float InScaleMax = 230;    // and Max for both    //EDIT Set your window MAX TEMP here
float OutScaleMin = 0;      // the top and       
float OutScaleMax = 1000;    // bottom trends     //EDIT If you have used a different Window Size in the Arduino_PID code (default is 1000), match it here.


int windowSpan = 3000000;    // number of mS into the past you want to display    //EDIT Set how far you want to look back here in ms
int refreshRate = 100;      // how often you want the graph to be reDrawn;

//float displayFactor = 1; //display Time as Milliseconds    //EDIT If you are controlling an extremely fast process, uncomment this line
//float displayFactor = 1000; //display Time as Seconds      //EDIT If you are controlling an pretty fast process, uncomment this line
float displayFactor = 60000; //display Time as Minutes      //EDIT If you uncomment either of the above 2 lines, comment this line out
...
ControlP5 controlP5;
controlP5.Button AMButton, DRButton, TuningButton;
...
void setup()
{
  ...
  //KP prints serial list to the console window. Select the correct value in the myPort line by changing the [0] value.
  ...
  TuningButton = controlP5.addButton("Toggle_Tuning",0.0,10,475,70,20); //KP Place one more button to set Tuning On/Off button.
  TuningLabel = controlP5.addTextlabel("Tuning","Off",12,497); 
  TuningCurrent = controlP5.addTextlabel("TuningCurrent","Off",80,490); 

  controlP5.addButton("Send_To_Arduino",0.0,10,525,120,20); 
...
void Toggle_Tuning() { //KP Adding Toggle Tuning On/Off
  if(TuningLabel.valueLabel().getText()=="Off") 
  {
...
void Send_To_Arduino()
{
  ...
  Byte t = (TuningLabel.valueLabel().getText() =="Off")?(byte)0:(byte)1; //KP Read On/Off state of Tuning Toggle, set t to correct value.
  myPort.write(a);
  myPort.write(d);
  myPort.write(t);
...
void serialEvent(Serial myPort)
{
  ...
  if (s.length == 10) //KP Read to one more number
  {
    ...
    TuningCurrent.setValue(trim(s[9])); //KP Set Tuning Parameter
